...
Return `Tuple(xmean, ycov)`, where `mean` is mean value, `std` means standard deviation.
...
"""
$(TYPEDSIGNATURES)
Compute distribution of skewness between `y1`, `y2` and `y3` along `x`. For example, rotation velocity along radius.
Return `Tuple(xmean, yskewness)`, where `mean` is mean value, `std` means standard deviation.

## Keywords
$_common_keyword_section
$_common_keyword_head_tail
"""
function distribution_skewness(x::Array, y1::Array, y2::Array, y3::Array, weight=nothing;
        section::Int64 = 10,
        rmhead::Int64 = 0,
        rmtail::Int64 = 0,
        uniform_interval::Bool = true,
    ...
    length(x) == length(y1) || throw(DimensionMismatch("x and y1 must have the same length"))
    length(x) == length(y2) || throw(DimensionMismatch("x and y2 must have the same length"))
    length(x) == length(y3) || throw(DimensionMismatch("x and y3 must have the same length"))
    ...
    data = StructArray(DataSkewnessWeighted.(
        x[1+rmhead:end-rmtail],
        y1[1+rmhead:end-rmtail],
        y2[1+rmhead:end-rmtail],
        y3[1+rmhead:end-rmtail],
        weights[1+rmhead:end-rmtail]
    ))
    N = length(data)
    if N == 0
    ...
    if uniform_interval
        ...
        for i in 1:section
            ...
            if isempty(bin)
                ...
                y3slice = identity.(collect(bin.y3))
                y3mean = mean(y3slice, weight_slice)

                push!(yskewness, sum((y1slice .- y1mean) .* (y2slice .- y2mean) .* (y3slice .- y3mean) .* weight_slice) / sum(weight_slice))
                # push!(yskewness, sum(y1slice .* (y2slice .- y2mean) .* (y3slice .- y3mean) .* weight_slice) / sum(weight_slice))
                # push!(ymean, mean(yslice, weight_slice))
                # push!(ystd, std(yslice, weight_slice))
            end
    ...
    if counts
        return xmean, yskewness, bin_N
    else
        return xmean, yskewness
    end
end
